...
section('weekly swap leaderboard');
{
  ...
  const boardRes = await call(env, 'GET', '/api/swap/leaderboard');
  eq('the board still answers once there are badges to work out', boardRes.status, 200);
  const badged = boardRes.body.top;
