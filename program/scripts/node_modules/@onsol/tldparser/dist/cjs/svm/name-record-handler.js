 ...
 * @returns
 */
const getDomainKey = async (domainTld, record = false) => {
    ...
    if (domainTldSplit.length === 3) {
        ...
        const { pubkey: subKey } = await _getNameAccount('\0'.concat(subDomain), domainKey);
        // Sub record
...
const _getNameAccount = async (name, parent) => {
    if (!parent) {
        parent = await (0, utils_1.getOriginNameAccountKey)();
    }
    let hashed = await (0, utils_1.getHashedName)(name);
    let [pubkey] = await (0, utils_1.getNameAccountKeyWithBump)(hashed, undefined, parent);
