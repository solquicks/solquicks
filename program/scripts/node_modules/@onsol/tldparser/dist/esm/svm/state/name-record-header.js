...
export class NameRecordHeader {
    ...
    static async create(obj, connection, parentNameRecord) {
    ...
    async initializeParentNameRecordHeader(connection) {
    ...
    updateGracePeriod(parentNameRecord) {
     ...
     */
    static async fromAccountAddress(connection, nameAccountKey) {
        const nameAccount = await connection.getAccountInfo(nameAccountKey, 'confirmed');
        if (!nameAccount) {
            return undefined;
        }
        const decodedData = deserialize(this.schema, Uint8Array.from(nameAccount.data));
     ...
     * the {@link NameRecordHeader} from its data.
     */
    static async fromMultipileAccountAddresses(connection, nameAccountKey) {
        let nameRecordAccountInfos = await connection.getMultipleAccountsInfo(nameAccountKey);
        let nameRecords = [];
    ...
    static fromAccountInfo(nameAccountAccountInfo) {
        const decodedData = deserialize(this.schema, Uint8Array.from(nameAccountAccountInfo.data));
        const res = new NameRecordHeader(decodedData);
