'use strict';
import { Contract, ensNormalize, namehash as ensNamehash, Typed, } from 'ethers';
...
async function getNameData(params) {
    ...
    const contract = new Contract(registrarAddress, ['function nameData(string) view returns ((string, uint256, bool, string, address))'], provider);
...
async function getScData(params) {
    ...
    if (!registrarAddress)
        throw Error('No registrar address');
    const contract = new Contract(registrarAddress, [
...
async function getUsersNfts(params) {
    ...
    const nftData = await Promise.all(nfts.map(async (tokenId) => {
        const tokenDataRaw = (await contract.nameData(Typed.uint256(tokenId)));
        return {
            name: tokenDataRaw[0],
...
async function getMainDomainRaw(params) {
    ...
    try {
        const reverseNode = ensNamehash(address.substring(2).toLowerCase() + '.addr.reverse');
        const resolverAddress = '0x314582158A0a72802aD8F6EeE6243C73dCf1F562';
        const resolverContract = new Contract(resolverAddress, ['function name(bytes32) view returns (string)'], provider);
